---
title: Cooking Classes for Datatype Generic Programming
date: March 3, 2016
---
...
SPJ in 2003. Generics fall out of the observation that all datatypes in Haskell
can be written as a combination of a *sum of products*.
...
What if we didn't toss out all this information and instead exposed it to our
program so that we could write generic logic that can introspect the "structure"
of our datatypes.
...
associate data type with typeclass. So the structure of our Generic class can
have ``Rep`` object which carries the structure of our datatype.
...
The instance here is purely and can be derived from GHC's internal
...
and
[GHC.TypeCon](https://downloads.haskell.org/~ghc/7.10.2/docs/html/libraries/ghc-7.10.2/TyCon.html). And the 
...
For example for ``Animal`` example from before we'd have several constructor
instances automatically generated by which we could query the names from the
AST.

```haskell
type Rep Animal = M1 D (T_Animal ((M1 C (C_Dog U1)) :+: (M1 C (C_Cat U1))))
...
**zero-cost abstraction** that has no overhead. We're simply making more
...
command line parsers) that are well-documented elsewhere on the web. So let's
consider an example based on the silly pun in the title of this article, namely
cooking typeclasses.
...
put on a menu from the structure of the Haskell types. Records will denote
named variations ("filling" vs "topping") of the menu item, while sum denote the
various options on the variations ("cherry" vs "apple").
...
new ``Crisp`` ( my favorite!) desert and we get everything for free.
